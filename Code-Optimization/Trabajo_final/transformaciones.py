    ...
    for elemento_herramienta in registros:
        if elemento_herramienta.get('id','Clave no encontrada')== id:
            if elemento_herramienta.get('cantidad')>=elemento.get('cantidad','Clave no encontrada'):
                ...
                print(f'Se acepta la solicitud debido a que hay stock de la herramienta, y queda un total de {elemento_herramienta.get('cantidad','clave no encontrada')} unidades de esa herramienta en Stock')
